...
export const fetchApplication = createAsyncThunk('applications/fetchApplication', async (applicationId, { rejectWithValue }) => {
    try {
...
export const fetchApplicationInPosting = createAsyncThunk('applications/fetchApplicationByPosing', async (postingId, { rejectWithValue }) => {
    try {
        const token = localStorage.getItem('token');
        if (!token) {
            throw new Error('Token không tồn tại');
...
export const updateApplication = createAsyncThunk('applications/updateApplication', async ({ cvId, updatedCV }, { rejectWithValue }) => {
    try {
        const token = localStorage.getItem('token');
        if (!token) {
            throw new Error('Token không tồn tại');
